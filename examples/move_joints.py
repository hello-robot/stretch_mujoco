...
def lift_sequence():
    ...
    sim.move_to(Actuators.lift, LIFT_START_POS, timeout=5.0)
    ...
    sim.move_by(Actuators.lift, MOVE_LIFT_BY, timeout=5.0)

    current_lift_position = sim.pull_status().lift.pos
    
    if not np.isclose(start_lift_position, current_lift_position, atol=0.05):
        print(f"The lift did not move by the specified amount. Asked to move from {start_lift_position:.2f} by {MOVE_LIFT_BY}, but ended up at {current_lift_position:.2f}. Should be {start_lift_position + MOVE_LIFT_BY :.2f}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        while sim.is_running():
            ...
            sim.move_to(Actuators.head_pan, random.random()-0.5, timeout=None)
            sim.move_to(Actuators.head_tilt, random.random()-0.5, timeout=2.0)
